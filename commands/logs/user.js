...
function _execute(message, user) {
    ...
    db.query("SELECT DISTINCT(idChannel) AS `channel_id`, ( SELECT count(*) FROM `MessageLogs` WHERE `MessageLogs`.`idChannel` = `channel_id` AND `MessageLogs`.`idMember` = ? AND `MessageLogs`.`idServer` = ?) as msgTotal, ( SELECT count(*) FROM `MessageLogs` WHERE `MessageLogs`.`idChannel` = `channel_id` AND `MessageLogs`.`idMember` = ? AND `MessageLogs`.`idServer` = ? AND `MessageLogs`.`sendTime` >= ? ) as msgToday FROM `MessageLogs` WHERE `MessageLogs`.`idMember` = ? AND `MessageLogs`.`idServer` = ? AND `MessageLogs`.`sendTime` >= ? ORDER BY `msgToday` DESC LIMIT 10", [user.user.id, message.guild.id, user.user.id, message.guild.id, timeFormat.getTodayTimeStamp(), user.user.id, message.guild.id, timeFormat.getTodayTimeStamp()], result => {
